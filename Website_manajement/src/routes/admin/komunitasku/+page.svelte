<script>
	...
	async function fetchdata() {
		try {
			...
			const komunitasData = await response.json();
			komunitas = komunitasData.data;

			const anggotaResponse = await fetch('http://localhost:8080/anggotakomunitas/pending');
	...
	async function updateStatusRequest(request, statusBaru) {
		try {
			...
			const response = await fetch('http://localhost:8080/anggotaKomunitas/updatestatus', {
				...
				body: JSON.stringify(anggotaKomunitas)
			});
...
<div class="h-screen w-screen flex justify-center items-center bg-background">
	...
	<div class="flex flex-col space-y-5">
		{#each anggota as item}
			...
			>
				<div class="flex flex-col sm:w-2/3 w-full">
					<h1 class="text-lg md:text-xl text-center font-bold">{item.id_komunitas}</h1>
				...
				<div class="flex justify-between items-center">
					<div class="flex justify-between items-center">
						<div class="flex items-center">
							<button on:click={() => updateStatusRequest(request, 'diterima')}>
								<svg
						...
						<div class="flex items-center">
							<button on:click={() => updateStatusRequest(request, 'ditolak')}>
								<svg
									xmlns="http://www.w3.org/2000/svg"
									width="2.5em"
									height="2.5em"
